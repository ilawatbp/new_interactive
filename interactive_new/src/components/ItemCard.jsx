...
export default function SkelitonCard({ items }) {
  ...
  return (
    <>
      {/* modal div */}
      <div className={`fixed w-full h-[100dvh] top-0 bg-black/90 flex justify-center items-center 
                        ${modalOpen ? 'opacity-1 z-50' : 'opacity-0 z-0'}
                        transition-all duration-200 ease-in
                        `}
            onClick={()=>setModalOpen(false)}
      >
        <div className="w-[40%] bg-[#e7e5e5] px-4 py-10 rounded-2xl flex flex-col items-center"
          onClick={(e)=> e.stopPropagation()}
        >
          <img  src={`itemImage/${selectedItem.ItemCode}.webp`} alt="" className=" rounded-2xl" />
          <div className="flex-[1] rounded-2xl w-full  px-6 flex flex-col justify-center mt-2 gap-1">
            ...
            <div className="flex justify-end gap-4 items-center">
              <ShoppingBasket
                ...
                // 🧩 added: cursor-pointer (same as above)
              />
            </div>
          </div>
        </div>
      ...
      <div
        className={`w-full px-20 py-4 flex gap-4 flex-wrap justify-evenly pt-20`}
      >
        {items.map((itm, index) => (
          <div
            key={index}
